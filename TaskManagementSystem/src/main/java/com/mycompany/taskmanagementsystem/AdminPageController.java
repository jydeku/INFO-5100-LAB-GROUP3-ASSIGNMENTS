...
public class AdminPageController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        assignedToColumn.setCellValueFactory(new PropertyValueFactory<Task, String>("user"));
    ...
    private void loadTasksFromFile() {
        try (BufferedReader reader = new BufferedReader(new FileReader("user.txt"))) {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                String assignedTo = taskData[5];
                
            Task task = new Task(taskTitle, description, priority, deadline, status, assignedTo);
...
private void setupTableColumns() {
    ...
    private void editData(){
        ...
        assignedToColumn.setCellFactory(TextFieldTableCell.<Task>forTableColumn());
        assignedToColumn.setOnEditCommit(event ->{
            Task task = event.getTableView().getItems().get(event.getTablePosition().getRow());
            task.setUser(event.getNewValue());
            //System.out.println(task.getTaskName()+ "was updated to "+ event.getNewValue() +" at row "+ (event.getTablePosition().getRow() + 1));
        });
    }
